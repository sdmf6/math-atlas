...
export default function ExamBasketPage() {
  ...
  const [usePandocFallback, setUsePandocFallback] = useState(false);
  ...
  // ===== 浏览器端生成 Word (降级方案) =====
  const downloadWordClient = async () => {
    if (questions.length === 0) { setMessage('未识别到题目'); return; }
    const allContents = await fetchMissing(exportQuestions.map(q => q.qid));
  ...
  const downloadWord = async () => {
    ...
    try {
      // 尝试 Pandoc 导出
      if (!usePandocFallback) {
        try {
          setMessage('⏳ 正在使用 Pandoc 生成 Word 文档...');

          const res = await fetch('/api/export-docx', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              qids: exportQuestions.map(q => q.qid)
            }),
          });
          ...
          if (res.ok) {
            ...
            setMessage(`✅ 已导出 ${exportQuestions.length} 道题目为 Word 文档 (Pandoc)`);
            return;
          }

          const data = await res.json().catch(() => ({}));
          throw new Error(data.error || 'Pandoc 导出失败');

        } catch (pandocError: any) {
          console.warn('Pandoc 导出失败:', pandocError.message);

          if (pandocError.message.includes('Pandoc') ||
            pandocError.message.includes('ENOENT') ||
            pandocError.message.includes('pandoc')) {
            setMessage('⚠️ Pandoc 不可用，切换到浏览器端生成...');
            setUsePandocFallback(true);
          } else {
            throw pandocError;
          }
        }
      ...
      // 降级到浏览器端生成
      setMessage('⏳ 使用浏览器端生成 Word 文档...');
      await downloadWordClient();
      setMessage(`✅ 已导出 ${exportQuestions.length} 道题目为 Word 文档 (浏览器端)`);

    } catch (e: any) {
      console.error('Word 导出失败:', e);
      // 给出更友好的错误信息
      if (e.message.includes('ByteString')) {
        setMessage('❌ 导出失败：文件名包含特殊字符，已自动修复，请重试');
      } else {
        setMessage('❌ 导出失败：' + e.message);
      }
    } finally {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.metaBar}>
        <button className={styles.saveBtn} onClick={copyAsMarkdown} style={{ background: '#4a90d9' }}>
          📋 复制为 Markdown
        </button>
        <button className={styles.saveBtn} onClick={downloadZip} style={{ background: '#4a90d9' }}>
          📦 打包下载 Markdown (.zip)
        </button>
        <button className={styles.saveBtn} onClick={copyAsLatex} style={{ background: '#7b68ee' }}>
          📐 复制为 LaTeX
        </button>
        <button className={styles.saveBtn} onClick={downloadLatexZip} style={{ background: '#7b68ee' }}>
          📦 打包下载 LaTeX (.zip)
        </button>
        <button className={styles.saveBtn} onClick={exportToLocal} style={{ background: '#7b68ee' }}>
          📁 LaTeX 导出到本地
        </button>
        <button
        ...
        >
          {exportingWord ? '⏳ 导出中...' : '📄 Word 导出'}
        </button>
        <button className={styles.saveBtn} onClick={clearAll} style={{ background: '#95a5a6' }}>
          🗑️ 清空试题栏
        </button>
        <button
          onClick={() => router.push('/')}
                ...
                >
                  <div style={{ position: 'absolute', top: '8px', right: '8px', display: 'flex', gap: '6px', zIndex: 10 }}>
                    <button
                      ...
                      title="上移"
                      style={{
                        padding: '4px 10px', fontSize: '0.8rem', border: '1px solid #ccc',
                        borderRadius: '6px', background: '#fff',
                        cursor: i === 0 ? 'not-allowed' : 'pointer', opacity: i === 0 ? 0.3 : 0.85,
                      }}
                    >↑</button>
                    <button
                    ...
                    <button
                      ...
                      title="移除此题"
                      style={{
                        padding: '4px 12px', fontSize: '0.8rem', border: '1px solid #e74c3c',
                        borderRadius: '6px', background: '#fff', color: '#e74c3c',
                  ...
                  <div style={{
                    ...
                    fontSize: '0.7rem', color: '#999', zIndex: 10, pointerEvents: 'none',
                  }}>
                    ⠿ 拖动排序
                  </div>
